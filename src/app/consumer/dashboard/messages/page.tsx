        ...
        <div className="w-72 shrink-0 border-l border-gray-100 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">
              ...
              <p className="text-xs mt-1">تصفح المصورين وابدأ محادثة</p>
            </div>
          ) : (
            <div className="p-2">
              {conversations.map((conv) => (
                ...
                >
                  {conv.photographer.user.avatarUrl ? (
                    <img
                      src={conv.photographer.user.avatarUrl}
                      alt={conv.photographer.user.name}
                    ...
                    <div className="w-10 h-10 rounded-xl bg-[#C0A4A3] flex items-center justify-center text-white font-bold text-sm shrink-0">
                      {conv.photographer.user.name[0]}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-800 text-sm">{conv.photographer.user.name}</p>
                    {conv.messages[0] && (
